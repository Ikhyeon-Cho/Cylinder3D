...
class AsymmResBlock(BaseBlock):
    ...
    def forward(self, x):
        ...
        x2 = self.residual(x)
        x1.features = x1.features + x2.features
...
class DownsamplingAsymmResBlock(BaseBlock):  # with pooling
    ...
    def forward(self, x):
        # Residual connection
        x1: spconv.SparseConvTensor = self.skip_connection(x)
        x2: spconv.SparseConvTensor = self.residual(x)
        x1.features = x1.features + x2.features

        # Optional pooling
        if self.has_pooling_layer:
            return self.pooling(x1), x1
        else:
            return x1
