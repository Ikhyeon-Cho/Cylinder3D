...
if __name__ == "__main__":
    model = Asymm_3d_spconv(output_shape=[11, 256, 256],
                            num_input_features=128,
                            nclasses=20,
                            n_height=32,
                            init_size=16)
    ...
    # Random voxel features
    voxel_features = torch.randn(num_points, 128)  # (N, C)

    # Random coordinates in range [0, output_shape)
    coors = torch.stack([
        torch.randint(0, batch_size, (num_points,)),  # batch idx
        torch.randint(0, 11, (num_points,)),          # z
        torch.randint(0, 256, (num_points,)),         # y
        torch.randint(0, 256, (num_points,))          # x
    ], dim=1)

    # Forward pass
    output = model(voxel_features, coors, batch_size)
    print(f"Input shape: {voxel_features.shape}")
    print(f"Output shape: {output.shape}")  # Should be [2, 20, 11, 256, 256]
